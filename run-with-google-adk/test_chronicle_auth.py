...
class ChromicleAuthTester:
    ...
    def test_google_auth(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            is_impersonated = "impersonated_credentials" in str(type(credentials))
            ...
            if cred_info["service_account"]:
                ...
                if is_impersonated:
                    self.log("Using impersonated service account credentials")
            else:
                self.log("Using user credentials (ADC)")

            # Test token refresh if needed - but skip for impersonated credentials
            # as they handle refresh differently
            if hasattr(credentials, 'valid') and not credentials.valid and not is_impersonated:
                self.log("Refreshing credentials...")
                try:
                    credentials.refresh(Request())
                    cred_info["valid"] = credentials.valid
                except Exception as refresh_error:
                    self.log(f"Note: Credential refresh raised: {refresh_error}", "WARNING")
                    cred_info["valid"] = "refresh_failed"
            elif is_impersonated:
                # For impersonated credentials, just note they're configured
                self.log("Impersonated credentials configured (refresh handled automatically)")
                cred_info["valid"] = True
...
Examples:
  ...
  # First set PROJECT_ID: export PROJECT_ID=$(grep GOOGLE_CLOUD_PROJECT agents/google_mcp_security_agent/.env | cut -d'=' -f2)
...
Environment Variables:
  ...
  CHRONICLE_REGION       - Optional: Chronicle region (default: us)
    ...
    # Load configuration
    _ = load_configuration(args.env_file, args.no_env_file)
